...
- (void)updateBounds {
    ...
    NSLog(@"heightRation=%ld",self.HeightRatio);
    NSLog(@"widhtRation=%ld",self.WithRatio);
    
    
        NSLog(@"width=%f",height);
        NSLog(@"height=%f",width);
    
    //width＝height*3/5;
    //height=width/32*17;
//height=width*self.HeightRatio/self.WithRatio;
    
    if (height!=0&&width!=0&&self.WithRatio!=0&&self.HeightRatio) {
        height=width*self.HeightRatio/self.WithRatio;        
    }
...
//    NSLog(@"height=%f",crop.size.height);
...
+ (UIView *)initialCropViewForImageView:(UIImageView*)imageView
                         andWidthRation:(NSInteger) widthRation
                        andHeightRation:(NSInteger) heightRation {
...
//    CGFloat height = CGHeight(max) / 4 * 3;
    ...
    NSLog(@"heightRation=======%ld",(long)heightRation);
    NSLog(@"widthRation=======%ld",(long)widthRation);
    
    if (heightRation!=0&&widthRation!=0) {
        height=width*heightRation/widthRation;
    }
...
- (void)setup {
    ...
    NSLog(@"================withRatio=%ld",self.WithRatio);
    NSLog(@"================heightRatio=%ld",self.HeightRatio);

    cropView = [BJImageCropper initialCropViewForImageView:imageView
                                            andWidthRation:self.WithRatio
                                           andHeightRation:self.HeightRatio];
...
- (id)initWithImage:(UIImage *)newImage
      ...
      andWidthRatio:(NSInteger)widthRatio
     andHeightRatio:(NSInteger)heightRatio{
    ...
    switch ([allTouches count])
    {
        case 1: {
            ...
            if (isPanning) {
                ...
                //height=width*self.HeightRatio/self.WithRatio;
                //width=height*self.WithRatio/self.HeightRatio;
                height=width*self.HeightRatio/self.WithRatio;
...
//                width=height*1/1;
                ...
                cropView.frame = frame;
                NSLog(@"width after moved:%f",frame.size.width);
                NSLog(@"height after moved:%f",frame.size.height);
            }
        } break;
        case 2: {
            CGPoint touch1 = [[[allTouches allObjects] objectAtIndex:0] locationInView:self.imageView];
